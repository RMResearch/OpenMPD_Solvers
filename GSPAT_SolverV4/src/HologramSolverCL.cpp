...
{ 
	...
	GSPAT_V4::RegisterPrintFuncs(printConsole, printConsole, printError);
...
HologramSolverCL::~HologramSolverCL() {
	...
	delete solutionPool;
	GSPAT_V4::printMessage_GSPAT("GSPAT: Thread pool succesfully destroyed");
	//2. Delete buffers
	cl_uint count;
	if (configured) {
		...
		GSPAT_V4::printMessage_GSPAT("GSPAT: Board config (mappings and phase delays) succesfully destroyed");
		configured = false;
	}
	...
	GSPAT_V4::printMessage_GSPAT("GSPAT: Kernels succesfully destroyed");
	//4. Delete programs
	clReleaseProgram(program);
	GSPAT_V4::printMessage_GSPAT("GSPAT: Program succesfully destroyed");
	//5. Delete clBlas, queue and context
	clblasTeardown();
	GSPAT_V4::printMessage_GSPAT("GSPAT: clBlas succesfully destroyed");
	//Delete global context
	clReleaseCommandQueue(queue);
	clReleaseContext(context);
	GSPAT_V4::printMessage_GSPAT("GSPAT: Context destroyed");
...
void HologramSolverCL::setBoardConfig(float* transducerPositions, int* transducerToPINMap, int* phaseAdjust, float* amplitudeAdjust, int numDiscreteLevels ) {
	...
	if (configured) {
		...
		clReleaseMemObject(phaseCorrections);
		configured = false;
		GSPAT_V4::printMessage_GSPAT("GSPAT: Discarding previous configuration (transducer IDs, phase adjustments)\n");
	...
	this->numDiscreteLevels = numDiscreteLevels;
	GSPAT_V4::printMessage_GSPAT("GSPAT: Board configured (transducer positions, transducerIDs, phase Adjustments)\n");
...
void HologramSolverCL::initializeOpenCL(){
	...
	OpenCLUtilityFunctions::createProgramFromFile("hologramSolver_V4.cl", context, deviceUsed, &program, compilerOptions);
	cl_int err;
	fillAllPointHologramsKernel = clCreateKernel(program, "computeFandB", &err);
    if(err < 0) {
		GSPAT_V4::printError_GSPAT("GSPAT: Couldn't create kernel\n");
    }
	powerMethodKernel = clCreateKernel(program, "solvePhases_GS", &err);
	if(err < 0) {
		sprintf(consoleLine,"GSPAT: Couldn't create a kernel: %d", err);
		GSPAT_V4::printError_GSPAT(consoleLine);
	}
...
{
	//static const float max_angle = 2 * M_PI* 24.1f;//Limit according to Space-Bandwidth product (sin(max_angle) = lambda/(2*transducerPitch))
	static const float max_angle = M_PI/3;			//Approximating max_angle to 30 degrees
	static const float min_cos = cosf(max_angle);
	cl_int error;
	//Create 1D texture for transducer's directivity: 
	{
		...
		for (size_t p = 0; p < imageWidth - 1; p++) {
			//Each pixel in the texture [0...imageWidth) represents a angular direction (in terms of the cos(angle) in [0..1)) 
			float cos_value = (1.0f*p) / (imageWidth - 1);
			//The intensity for that direction is computed from piston model (unless angle above Space-bandwidth product).
			if (cos_value < min_cos)
				directivityBuffer[p] = 0;//Above Space-bandwidth product... ignore
			else
				directivityBuffer[p] = computeDirectivityForCosAlpha(cos_value);//All good-> piston model
		}
		directivityBuffer[imageWidth - 1] = P_ref;// Our piston model function fails for an input of exactly 1... so we set it manually.
	...
	if (error == NULL ) {
		GSPAT_V4::printMessage_GSPAT("GSPAT: Directivity bufffers created successfully\n");
	}
	else 
		GSPAT_V4::printError_GSPAT("GSPAT: Could not create directivity buffers\n");
}
...
void HologramSolverCL:: createTransducerPositionBuffer(float* transducerPositions) {
		...
		if (err < 0) { GSPAT_V4::printError_GSPAT("GSPAT: Couldn't create 'transducerPositions' buffer"); return ; }
		...
		if (err < 0) { GSPAT_V4::printError_GSPAT("GSPAT: Couldn't write 'transducerPositions' buffer"); return ; }
		delete positions;
		//BEGIN DEBUG
...
void HologramSolverCL::createPointsInductionBuffer() {
	points_Re_Im_induction = clCreateBuffer(context, CL_MEM_READ_WRITE, HologramSolution::MAX_POINTS*HologramSolution::MAX_GEOMETRIES * 2 * sizeof(float), NULL,NULL);
	float zero_phase[2] = { 1.0f,  0.0f };
	clEnqueueFillBuffer(queue, points_Re_Im_induction, zero_phase, 2 * sizeof(float), 0, HologramSolution::MAX_POINTS*HologramSolution::MAX_GEOMETRIES * 2 * sizeof(float), 0, NULL, 0);
}
...
Solution* HologramSolverCL::createNewDividerSolution(unsigned char FPSDivider) {
	if (FPSDivider == 0)
		GSPAT_V4::printWarning_GSPAT("GS_PAT: Incorrect divider value (zero). Command ignored (previous dividir still in use).");
	HologramSolution* nextSolution;
	//1. Access next available solution from the pool (and remove it from the list of "availables")
	nextSolution = solutionPool->getNextSolution();
...
void HologramSolverCL::updateCLBuffers(HologramSolution * hs)
{
	...
	if (hs->manualData) {
		...
		if (err < 0) { sprintf(consoleLine, "GSPAT::updateCLBuffers::Couldn't copy input parameters to the Solution"); GSPAT_V4::printWarning_GSPAT(consoleLine); return; }
	}
}
...
void HologramSolverCL::computeFandB(HologramSolution* solution) {
	...
	if (err < 0) { GSPAT_V4::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 0"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 1, sizeof(cl_mem), &(solution->positions_CLBuffer));
	if (err < 0) { GSPAT_V4::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 1"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 2, sizeof(cl_mem), &(solution->matrixStart_CLBuffer));
	if (err < 0) { GSPAT_V4::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 2"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 3, sizeof(cl_mem), &(solution->matrixEnd_CLBuffer));
	if (err < 0) { GSPAT_V4::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 3"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 4, sizeof(int), &(solution->numPoints));
	if (err < 0) { GSPAT_V4::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 4"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 5, sizeof(int), &(solution->numGeometries));
	if (err < 0) { GSPAT_V4::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 5"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 6, sizeof(cl_mem), &(this->directivityTexture));
	if (err < 0) { GSPAT_V4::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 6"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 7, sizeof(cl_mem), &(solution->singlePointField_CLBuffer));
	if (err < 0) { GSPAT_V4::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 7"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 8, sizeof(cl_mem), &(solution->singlePointFieldNormalised_CLBuffer));
	if (err < 0) { GSPAT_V4::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 8"); return ; }
	...
	if (err < 0) { 
		GSPAT_V4::printError_GSPAT("GSPAT: computeFandB:: Couldn't enqueue the kernel"); return ; 
	}	
...
void HologramSolverCL::computeR(HologramSolution* solution,bool useDiego) {
	...
	for (int g = 0; g < solution->numGeometries; g++) {
		...
		if (retCode != clblasStatus::clblasSuccess) {
			sprintf(consoleLine, "GSPAT: computeR::CL_BLAS error: %d", retCode); GSPAT_V4::printError_GSPAT(consoleLine);
		}
	}
}
...
void HologramSolverCL::solvePhases_GS(HologramSolution* solution,bool useNorm){
	cl_int err;
	size_t global_size[2] = { (size_t)solution->numPoints, 1 }, local_size[2] = { (size_t)solution->numPoints, 1 };

	//1. Set arguments
	err = clSetKernelArg(powerMethodKernel, 0, sizeof(cl_mem), &this->points_Re_Im_induction);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 0"); GSPAT_V4::printWarning_GSPAT(consoleLine); return; }	
	err = clSetKernelArg(powerMethodKernel, 1, sizeof(cl_mem), &solution->pointsReIm_CLBuffer);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 1"); GSPAT_V4::printWarning_GSPAT(consoleLine); return; }
	err = clSetKernelArg(powerMethodKernel, 2, sizeof(int), &(solution->numGeometries));
	if (err < 0) { GSPAT_V4::printWarning_GSPAT("GSPAT: solvePhases_GS::Couldn't set kernel argument 2"); return ; }	
	err = clSetKernelArg(powerMethodKernel, 3, sizeof(cl_mem), &solution->R_CLBuffer);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 3"); GSPAT_V4::printWarning_GSPAT(consoleLine); return; }
	err = clSetKernelArg(powerMethodKernel, 4, sizeof(cl_mem), &solution->targetAmplitudes_CLBuffer);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 4"); GSPAT_V4::printWarning_GSPAT(consoleLine); return; }
	err = clSetKernelArg(powerMethodKernel, 5, sizeof(cl_mem), &solution->amplitudesPerPoint);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 5"); GSPAT_V4::printWarning_GSPAT(consoleLine); return; }
	err = clSetKernelArg(powerMethodKernel, 6, sizeof(cl_mem), &solution->correction);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 6"); GSPAT_V4::printWarning_GSPAT(consoleLine); return; }
	...
	delete powerEvents;
	if (err < 0) { 
		GSPAT_V4::printError_GSPAT("GSPAT: solvePhases_GS:: Couldn't enqueue the kernel"); return; }	
	//DEBUG
...
void HologramSolverCL::computeActivation(HologramSolution* solution){
	...
	//B. Build hologram by adding point holograms (and applying phase to each point)
	{
		...
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 0");GSPAT_V4::printWarning_GSPAT(consoleLine); return; }
		err = clSetKernelArg(addHologramsKernel, 1, sizeof(cl_mem), &solution->singlePointFieldNormalised_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 1"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(addHologramsKernel, 2, sizeof(cl_mem), &solution->pointsReIm_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 2"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(addHologramsKernel, 3, sizeof(cl_mem), &solution->correction);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 3"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(addHologramsKernel, 4, sizeof(cl_mem), &solution->finalHologramPhases_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 4"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(addHologramsKernel, 5, sizeof(cl_mem), &solution->finalHologramAmplitudes_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 5"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(addHologramsKernel, 6, sizeof(cl_mem), &solution->finalHologramReIm_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 6"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		...
		if (err < 0) {
			GSPAT_V4::printError_GSPAT("GSPAT: computeActivation::Couldn't enqueue the kernel"); return;
		}
	}
}
...
void HologramSolverCL::discretise(HologramSolution* solution) {
	cl_int err;
	{
		...
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 0"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 1, sizeof(cl_mem), &solution->finalHologramPhases_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 1"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 2, sizeof(cl_mem), &solution->finalHologramAmplitudes_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 2"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 3, sizeof(float), &solvePhaseOnly);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 3"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 4, sizeof(cl_mem), &phaseCorrections);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 4"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 5, sizeof(cl_mem), &transducerMappings);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 5"); GSPAT_V4::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 6, sizeof(cl_mem), &solution->messagesArray_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 6"); GSPAT_V4::printMessage_GSPAT(consoleLine);return; }		
		...
		if (err < 0) {
			GSPAT_V4::printError_GSPAT("GSPAT: discretise::Couldn't enqueue the kernel"); return;
		}
		...
		if (err < 0) {
			GSPAT_V4::printError_GSPAT("GSPAT: discretise::Couldn't read message queue"); return;
		}		
	}
}
