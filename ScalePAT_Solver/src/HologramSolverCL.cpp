...
{ 
	...
	ScalePat::RegisterPrintFuncs(printConsole, printConsole, printError);
...
HologramSolverCL::~HologramSolverCL() {
	...
	delete solutionPool;
	ScalePat::printMessage_GSPAT("GSPAT: Thread pool succesfully destroyed");
	//2. Delete buffers
	cl_uint count;
	if (configured) {
		...
		ScalePat::printMessage_GSPAT("GSPAT: Board config (mappings and phase delays) succesfully destroyed");
		configured = false;
	}
	...
	ScalePat::printMessage_GSPAT("GSPAT: Kernels succesfully destroyed");
	//4. Delete programs
	clReleaseProgram(program);
	ScalePat::printMessage_GSPAT("GSPAT: Program succesfully destroyed");
	//5. Delete clBlas, queue and context
	clblasTeardown();
	ScalePat::printMessage_GSPAT("GSPAT: clBlas succesfully destroyed");
	//Delete global context
	clReleaseCommandQueue(queue);
	clReleaseContext(context);
	ScalePat::printMessage_GSPAT("GSPAT: Context destroyed");
...
void HologramSolverCL::setBoardConfig(float* transducerPositions, int* transducerToPINMap, int* phaseAdjust, float* amplitudeAdjust, int numDiscreteLevels ) {
	...
	if (configured) {
		...
		clReleaseMemObject(phaseCorrections);
		configured = false;
		ScalePat::printMessage_GSPAT("GSPAT: Discarding previous configuration (transducer IDs, phase adjustments)\n");
	...
	this->numDiscreteLevels = numDiscreteLevels;
	ScalePat::printMessage_GSPAT("GSPAT: Board configured (transducer positions, transducerIDs, phase Adjustments)\n");
...
void HologramSolverCL::createDirectivityTexture()
{
	...
	if (error == NULL ) {
		ScalePat::printMessage_GSPAT("GSPAT: Directivity bufffers created successfully\n");
	}
	else 
		ScalePat::printError_GSPAT("GSPAT: Could not create directivity buffers\n");
}
...
void HologramSolverCL:: createTransducerPositionBuffer(float* transducerPositions) {
		...
		if (err < 0) { ScalePat::printError_GSPAT("GSPAT: Couldn't create 'transducerPositions' buffer"); return ; }
		...
		if (err < 0) { ScalePat::printError_GSPAT("GSPAT: Couldn't write 'transducerPositions' buffer"); return ; }
		delete positions;
		//BEGIN DEBUG
...
Solution* HologramSolverCL::createNewDividerSolution(unsigned char FPSDivider) {
	if (FPSDivider == 0)
		ScalePat::printWarning_GSPAT("GS_PAT: Incorrect divider value (zero). Command ignored (previous dividir still in use).");
	HologramSolution* nextSolution;
	//1. Access next available solution from the pool (and remove it from the list of "availables")
	nextSolution = solutionPool->getNextSolution();
...
void HologramSolverCL::updateCLBuffers(HologramSolution * hs)
{
	...
	if (hs->manualData) {
		...
		if (err < 0) { sprintf(consoleLine, "GSPAT::updateCLBuffers::Couldn't copy input parameters to the Solution"); ScalePat::printWarning_GSPAT(consoleLine); return; }
	}
}
...
void HologramSolverCL::computeFandB(HologramSolution* solution) {
	...
	if (err < 0) { ScalePat::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 0"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 1, sizeof(cl_mem), &(solution->positions_CLBuffer));
	if (err < 0) { ScalePat::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 1"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 2, sizeof(cl_mem), &(solution->matrixStart_CLBuffer));
	if (err < 0) { ScalePat::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 2"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 3, sizeof(cl_mem), &(solution->matrixEnd_CLBuffer));
	if (err < 0) { ScalePat::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 3"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 4, sizeof(int), &(solution->numPoints));
	if (err < 0) { ScalePat::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 4"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 5, sizeof(int), &(solution->numGeometries));
	if (err < 0) { ScalePat::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 5"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 6, sizeof(cl_mem), &(this->directivityTexture));
	if (err < 0) { ScalePat::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 6"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 7, sizeof(cl_mem), &(solution->singlePointField_CLBuffer));
	if (err < 0) { ScalePat::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 7"); return ; }
	err = clSetKernelArg(fillAllPointHologramsKernel, 8, sizeof(cl_mem), &(solution->singlePointFieldNormalised_CLBuffer));
	if (err < 0) { ScalePat::printWarning_GSPAT("GSPAT: computeFandB::Couldn't set kernel argument 8"); return ; }
	...
	if (err < 0) { 
		ScalePat::printError_GSPAT("GSPAT: computeFandB:: Couldn't enqueue the kernel"); return ; 
	}	
...
void HologramSolverCL::computeR(HologramSolution* solution,bool useDiego) {
	...
	for (int g = 0; g < solution->numGeometries; g++) {
		...
		if (retCode != clblasStatus::clblasSuccess) {
			sprintf(consoleLine, "GSPAT: computeR::CL_BLAS error: %d", retCode); ScalePat::printError_GSPAT(consoleLine);
		}
	}
}
...
void HologramSolverCL::solvePhases_GS(HologramSolution* solution,bool useNorm){
	...
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 0"); ScalePat::printWarning_GSPAT(consoleLine); return; }
	err = clSetKernelArg(powerMethodKernel, 1, sizeof(cl_mem), &solution->R_CLBuffer);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 1"); ScalePat::printWarning_GSPAT(consoleLine); return; }
	err = clSetKernelArg(powerMethodKernel, 2, sizeof(cl_mem), &solution->targetAmplitudes_CLBuffer);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 2"); ScalePat::printWarning_GSPAT(consoleLine); return; }
	err = clSetKernelArg(powerMethodKernel, 3, sizeof(cl_mem), &solution->amplitudesPerPoint);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 3"); ScalePat::printWarning_GSPAT(consoleLine); return; }
	err = clSetKernelArg(powerMethodKernel, 4, sizeof(cl_mem), &solution->correction);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_GS::Couldn't set kernel argument 4"); ScalePat::printWarning_GSPAT(consoleLine); return; }
	...
	delete powerEvents;
	if (err < 0) { 
		ScalePat::printError_GSPAT("GSPAT: solvePhases_GS:: Couldn't enqueue the kernel"); return; }	
	//DEBUG
...
void HologramSolverCL::computeActivation(HologramSolution* solution){
	...
	//B. Build hologram by adding point holograms (and applying phase to each point)
	{
		...
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 0");ScalePat::printWarning_GSPAT(consoleLine); return; }
		err = clSetKernelArg(addHologramsKernel, 1, sizeof(cl_mem), &solution->singlePointFieldNormalised_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 1"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(addHologramsKernel, 2, sizeof(cl_mem), &solution->pointsReIm_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 2"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(addHologramsKernel, 3, sizeof(cl_mem), &solution->correction);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 3"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(addHologramsKernel, 4, sizeof(cl_mem), &solution->finalHologramPhases_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 4"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(addHologramsKernel, 5, sizeof(cl_mem), &solution->finalHologramAmplitudes_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 5"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(addHologramsKernel, 6, sizeof(cl_mem), &solution->finalHologramReIm_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: computeActivation::Couldn't set kernel argument 6"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		...
		if (err < 0) {
			ScalePat::printError_GSPAT("GSPAT: computeActivation::Couldn't enqueue the kernel"); return;
		}
	}
}
...
void HologramSolverCL::discretise(HologramSolution* solution) {
	cl_int err;
	{
		...
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 0"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 1, sizeof(cl_mem), &solution->finalHologramPhases_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 1"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 2, sizeof(cl_mem), &solution->finalHologramAmplitudes_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 2"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 3, sizeof(float), &solvePhaseOnly);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 3"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 4, sizeof(cl_mem), &phaseCorrections);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 4"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 5, sizeof(cl_mem), &amplitudeCorrections);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 5"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 6, sizeof(cl_mem), &transducerMappings);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 6"); ScalePat::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 7, sizeof(cl_mem), &solution->messagesArray_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 7"); ScalePat::printMessage_GSPAT(consoleLine);return; }		
		...
		if (err < 0) {
			ScalePat::printError_GSPAT("GSPAT: discretise::Couldn't enqueue the kernel"); return;
		}
		...
		if (err < 0) {
			ScalePat::printError_GSPAT("GSPAT: discretise::Couldn't read message queue"); return;
		}		
	}
}
