...
public:
	HologramSolverCL(int width, int height, float transducerPitch = 0.0105f);
	~HologramSolverCL();
	virtual void* getSolverContext();
	virtual void setBoardConfig(int transducerIds[512],	int phaseAdjust[512], int numDiscreteLevels );
...
protected:
	...
	void createTransducerPositionBuffer(float pitch, int boardSize[2]);
