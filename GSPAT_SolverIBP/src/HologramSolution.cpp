...
float* HologramSolution::finalArrayPhases() {
	...
	if (transducersOffSolution) {
		GSPAT_IBP::printWarning_GSPAT("Trying to call 'finalArrayPhases' on a dummy (transducersOFF) solution. Ignored.");
		return NULL;
	}
	...
	if (err < 0) {
		GSPAT_IBP::printError_GSPAT("Couldn't read final hologram (phases)\n"); return NULL;
	}
...
float* HologramSolution::finalArrayAmplitudes() {
	if (transducersOffSolution) {
		GSPAT_IBP::printWarning_GSPAT("Trying to call 'finalArrayAmplitudes' on a dummy (transducersOFF) solution. Ignored.");
		return NULL;
	}
	...
	if (err < 0) {
		GSPAT_IBP::printError_GSPAT("Couldn't read final hologram (amplitudes)"); return NULL;
	}	
...
void HologramSolution::finalMessages(unsigned char** out) {
	if (!transducersOffSolution) {
		...
		if (err < 0) {
			GSPAT_IBP::printError_GSPAT("Couldn't read message queues"); ;
		}
	}
...
float* HologramSolution::finalHologramReIm() {
	if (transducersOffSolution) {
		GSPAT_IBP::printWarning_GSPAT("Trying to call 'finalHologramReIm' on a dummy (transducersOFF) solution. Ignored.");
		return NULL;
	}	
	...
	if (err < 0) {
		GSPAT_IBP::printError_GSPAT("Couldn't read final hologram"); return NULL;
	}
...
void HologramSolution::dataExternallyUploadedEvent(void* pExternalEvents, int numEvents) {
	if (numEvents != 1) {
		GSPAT_IBP::printError_GSPAT("HologramSolution:dataExternallyUploadedEvent-> Only one event expected.");
		return; 
	}
...
void HologramSolution::readPropagators(float* singlePointFields, float* singlePointFieldsNormalised, int numGeometries) {
	if (transducersOffSolution) {
		GSPAT_IBP::printWarning_GSPAT("Trying to call 'readPropagators' on a dummy (transducersOFF) solution. Ignored.");
		return ;
	}
...
void HologramSolution::readMatrixR(float* R, int numGeometries) {
	if (transducersOffSolution) {
		GSPAT_IBP::printWarning_GSPAT("Trying to call 'readMatrixR' on a dummy (transducersOFF) solution. Ignored.");
		return ;
	}
...
void HologramSolution::readTargetPointsReIm(float* targetPoints, int numGeometries) {
	if (transducersOffSolution) {
		GSPAT_IBP::printWarning_GSPAT("Trying to call 'readTargetPointsReIm' on a dummy (transducersOFF) solution. Ignored.");
		return ;
	}
