...
{ 
	...
	GSPAT_Naive::RegisterPrintFuncs(printConsole, printConsole, printError);
...
HologramSolverCL::~HologramSolverCL() {
	...
	delete solutionPool;
	GSPAT_Naive::printMessage_GSPAT("GSPAT: Thread pool succesfully destroyed");
	//2. Delete buffers
	cl_uint count;
	if (configured) {
		...
		GSPAT_Naive::printMessage_GSPAT("GSPAT: Board config (mappings and phase delays) succesfully destroyed");
		configured = false;
	}
	...
	GSPAT_Naive::printMessage_GSPAT("GSPAT: Kernels succesfully destroyed");
	//4. Delete programs
	clReleaseProgram(program);
	GSPAT_Naive::printMessage_GSPAT("GSPAT: Program succesfully destroyed");
	//5. Delete  queue and context
	clReleaseCommandQueue(queue);
	clReleaseContext(context);
	GSPAT_Naive::printMessage_GSPAT("GSPAT: Context destroyed");
...
void HologramSolverCL::setBoardConfig(float* transducerPositions, float* transducerNormals, int* transducerToPINMap, int* phaseAdjust, float* amplitudeAdjust, int numDiscreteLevels ) {
	...
	if (configured) {
		...
		clReleaseMemObject(phaseCorrections);
		configured = false;
		GSPAT_Naive::printMessage_GSPAT("GSPAT: Discarding previous configuration (transducer IDs, phase adjustments)\n");
	...
	this->numDiscreteLevels = numDiscreteLevels;
	GSPAT_Naive::printMessage_GSPAT("GSPAT: Board configured (transducer positions, transducerIDs, phase Adjustments)\n");
...
void HologramSolverCL::initializeOpenCL(){
	...
	OpenCLUtilityFunctions::createProgramFromFile("hologramSolver_Naive.cl", context, deviceUsed, &program, compilerOptions);
	cl_int err;
	fillAllPointHologramsKernel = clCreateKernel(program, "computeFandB", &err);
    if(err < 0) {
		GSPAT_Naive::printError_GSPAT("GSPAT: Couldn't create kernel\n");
    }
	solveNaiveKernel = clCreateKernel(program, "solvePhases_Naive", &err);
	if(err < 0) {
		sprintf(consoleLine,"GSPAT: Couldn't create a kernel: %d", err);
		GSPAT_Naive::printError_GSPAT(consoleLine);
	}
...
void HologramSolverCL::createDirectivityTexture()
{
	...
	if (error == NULL ) {
		GSPAT_Naive::printMessage_GSPAT("GSPAT: Directivity bufffers created successfully\n");
	}
	else 
		GSPAT_Naive::printError_GSPAT("GSPAT: Could not create directivity buffers\n");
}
...
void HologramSolverCL:: createTransducerPositionBuffer(float* transducerPositions) {
		...
		if (err < 0) { GSPAT_Naive::printError_GSPAT("GSPAT: Couldn't create 'transducerPositions' buffer"); return ; }
		...
		if (err < 0) { GSPAT_Naive::printError_GSPAT("GSPAT: Couldn't write 'transducerPositions' buffer"); return ; }
		delete positions;
		//BEGIN DEBUG
...
Solution* HologramSolverCL::createNewDividerSolution(unsigned char FPSDivider) {
	if (FPSDivider == 0)
		GSPAT_Naive::printWarning_GSPAT("GS_PAT: Incorrect divider value (zero). Command ignored (previous dividir still in use).");
	HologramSolution* nextSolution;
	//1. Access next available solution from the pool (and remove it from the list of "availables")
	nextSolution = solutionPool->getNextSolution();
...
void HologramSolverCL::updateCLBuffers(HologramSolution * hs)
{
	...
	if (hs->manualData) {
		...
		if (err < 0) { sprintf(consoleLine, "GSPAT::updateCLBuffers::Couldn't copy input parameters to the Solution"); GSPAT_Naive::printWarning_GSPAT(consoleLine); return; }
	}
}
...
void HologramSolverCL::computeFandB(HologramSolution* solution) {
	...
	if (err < 0) { 
		GSPAT_Naive::printError_GSPAT("GSPAT: computeFandB:: Couldn't enqueue the kernel"); return ; 
	}	
...
void HologramSolverCL::solvePhases_Naive(HologramSolution* solution){
	cl_int err;
	size_t global_size[2] = { (size_t)numTransducers, (size_t)solution->numGeometries }, local_size[2] = { (size_t)numTransducers, 1 };

	//1. Set arguments
	err = clSetKernelArg(solveNaiveKernel, 0, sizeof(int), &(solution->numPoints));
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_Naive::Couldn't set kernel argument 0");GSPAT_Naive::printWarning_GSPAT(consoleLine); return; }
	err = clSetKernelArg(solveNaiveKernel, 1, sizeof(cl_mem), &solution->pointsReIm_CLBuffer);
	...
	err = clSetKernelArg(solveNaiveKernel, 5, sizeof(cl_mem), &solution->finalHologramAmplitudes_CLBuffer);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_Naive::Couldn't set kernel argument 5"); GSPAT_Naive::printWarning_GSPAT(consoleLine);return; }
	err = clSetKernelArg(solveNaiveKernel, 6, sizeof(cl_mem), &solution->finalHologramReIm_CLBuffer);
	if (err < 0) { sprintf(consoleLine,"GSPAT: solvePhases_Naive::Couldn't set kernel argument 6"); GSPAT_Naive::printWarning_GSPAT(consoleLine);return; }
	...
	err |= clEnqueueNDRangeKernel(queue, solveNaiveKernel, 2, NULL, global_size, local_size, 1, &(solution->events[GSPAT_Event::F_AND_B_READY]), &(solution->events[GSPAT_Event::POINT_PHASES_READY]));
	if (err < 0) { 
...
void HologramSolverCL::discretise(HologramSolution* solution) {
	cl_int err;
	{
		...
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 0"); GSPAT_Naive::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 1, sizeof(cl_mem), &solution->finalHologramPhases_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 1"); GSPAT_Naive::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 2, sizeof(cl_mem), &solution->finalHologramAmplitudes_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 2"); GSPAT_Naive::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 3, sizeof(float), &solvePhaseOnly);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 3"); GSPAT_Naive::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 4, sizeof(cl_mem), &phaseCorrections);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 4"); GSPAT_Naive::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 5, sizeof(cl_mem), &amplitudeCorrections);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 5"); GSPAT_Naive::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 6, sizeof(cl_mem), &transducerMappings);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 6"); GSPAT_Naive::printWarning_GSPAT(consoleLine);return; }
		err = clSetKernelArg(discretiseKernel, 7, sizeof(cl_mem), &solution->messagesArray_CLBuffer);
		if (err < 0) { sprintf(consoleLine,"GSPAT: discretise::Couldn't set kernel argument 7"); GSPAT_Naive::printMessage_GSPAT(consoleLine);return; }		
		...
		if (err < 0) {
			GSPAT_Naive::printError_GSPAT("GSPAT: discretise::Couldn't enqueue the kernel"); return;
		}
