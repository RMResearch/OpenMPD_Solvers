...
void compareData(FILE* compareFile, float trapPosition[], float angle, bool corrected, float X_w[], float Y_w[], float Z_w[]) {
	...
	while (!feof(forcesFile)) {
		//2.1. Reading data
		float position_w[3], forceGorkov_w[3], forceGorkov_l[3], position_l[3];
		...
		//2.2. Compute position in local coords
		float auxPos[3];
		memcpy(auxPos, position_w, 3 * sizeof(float));
		auxPos[0] -= trapPosition[0]; auxPos[1] -= trapPosition[1]; auxPos[2] -= trapPosition[2];
		position_l[0] = auxPos[0]*X_w[0] + auxPos[1]*X_w[1] + auxPos[2]*X_w[2];
		position_l[1] = auxPos[0]*Y_w[0] + auxPos[1]*Y_w[1] + auxPos[2]*Y_w[2];
		position_l[2] = auxPos[0]*Z_w[0] + auxPos[1]*Z_w[1] + auxPos[2]*Z_w[2];
		//2.3. Filter data: 
		float distZ, distR;
		distZ = trapPosition[2] - position_w[2];
		distR = sqrtf((trapPosition[0] - position_w[0])*(trapPosition[0] - position_w[0])+(trapPosition[1] - position_w[1])*(trapPosition[1] - position_w[1]));
		//OUT FILE FORMAT: "X_w,Y_w,Z_w, dx_l, dy_l, dz_l, deltaZ, deltaR, GKx_w,GKy_w,GKz_w, GKx_l,GKy_l,GKz_l,\n";
		if(distZ<0.0025f && distZ>-0.0025f && distR<0.0045)
			fprintf(compareFile,"%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f, \n",
								trapPosition[0],trapPosition[1],trapPosition[2],
								position_l[0], position_l[1], position_l[2],
								distZ, distR,
								forceGorkov_w[0],forceGorkov_w[1], forceGorkov_w[2],
								forceGorkov_l[0],forceGorkov_l[1], forceGorkov_l[2]	);	
	}
...
void main(void) {
	...
	fprintf(compareFile, "X_w,Y_w,Z_w, dx_l, dy_l, dz_l, deltaZ, deltaR, GKx_w,GKy_w,GKz_w, GKx_l,GKy_l,GKz_l,\n");
