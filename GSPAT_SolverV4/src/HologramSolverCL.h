...
protected:
	...
	void createPointsInductionBuffer();
	/**
...
private: 
	...
	*/
	...
	/**
		points_Re_Im_induction: Contains the "initial guess" for each iteration. They are unitary (phase only points) and are updated
		to the phases used in each past iteration. This way, each solution starts from the phase values computed in the last solution 
		and computed only one GS iterations. This helps ensure "phase continuity" between updates. 
		Also, given the high computing rate and small changes (i.e. position displacement) between updates, 
		the solutions remain optimal. 
	*/
	cl_mem points_Re_Im_induction;
	/**
